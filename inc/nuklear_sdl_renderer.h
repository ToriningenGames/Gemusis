 ...
 * Edited by Toriningen to allow parallel operation
 */
/*
 * ==============================================================
 *
...
NK_API struct nk_context*   nk_sdl_init(struct nk_sdl *this, SDL_Window *win, SDL_Renderer *renderer);
NK_API void                 nk_sdl_font_stash_begin(struct nk_sdl *this, struct nk_font_atlas **atlas);
NK_API void                 nk_sdl_font_stash_end(struct nk_sdl *this);
NK_API int                  nk_sdl_handle_event(struct nk_sdl *this, SDL_Event *evt);
NK_API void                 nk_sdl_render(struct nk_sdl *this, enum nk_anti_aliasing);
NK_API void                 nk_sdl_shutdown(struct nk_sdl *this);
NK_API void                 nk_sdl_handle_grab(struct nk_sdl *this);
 ...
 * See discussion at https://discourse.libthis->org/t/rendergeometryraw-producing-different-results-in-metal-vs-opengl/34953
 */
...
NK_INTERN void
nk_sdl_device_upload_atlas(struct nk_sdl *this, const void *image, int width, int height)
{
    struct nk_sdl_device *dev = &this->ogl;

    SDL_Texture *g_SDLFontTexture = SDL_CreateTexture(this->renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STATIC, width, height);
...
NK_API void
nk_sdl_render(struct nk_sdl *this, enum nk_anti_aliasing AA)
{
    /* setup global state */
    struct nk_sdl_device *dev = &this->ogl;

    {
        ...
        nk_convert(&this->ctx, &dev->cmds, &vbuf, &ebuf, &config);

        /* iterate over and execute each draw command */
        offset = (const nk_draw_index*)nk_buffer_memory_const(&ebuf);

        clipping_enabled = SDL_RenderIsClipEnabled(this->renderer);
        SDL_RenderGetClipRect(this->renderer, &saved_clip);
#ifdef NK_SDL_CLAMP_CLIP_RECT
        SDL_RenderGetViewport(this->renderer, &viewport);
#endif

        nk_draw_foreach(cmd, &this->ctx, &dev->cmds)
        {
...

            {
                ...
                SDL_RenderGeometryRaw(this->renderer,
        ...
        SDL_RenderSetClipRect(this->renderer, &saved_clip);
        if (!clipping_enabled) {
            SDL_RenderSetClipRect(this->renderer, NULL);
        }
...
nk_sdl_init(struct nk_sdl *this, SDL_Window *win, SDL_Renderer *renderer)
{
    ...
    this->ctx.clip.copy = nk_sdl_clipboard_copy;
    this->ctx.clip.paste = nk_sdl_clipboard_paste;
    this->ctx.clip.userdata = nk_handle_ptr(0);
    nk_buffer_init_default(&this->ogl.cmds);
    return &this->ctx;
}

NK_API void
nk_sdl_font_stash_begin(struct nk_sdl *this, struct nk_font_atlas **atlas)
{
...
NK_API void
nk_sdl_font_stash_end(struct nk_sdl *this)
{
    const void *image; int w, h;
    image = nk_font_atlas_bake(&this->atlas, &w, &h, NK_FONT_ATLAS_RGBA32);
    nk_sdl_device_upload_atlas(this, image, w, h);
    nk_font_atlas_end(&this->atlas, nk_handle_ptr(this->ogl.font_tex), &this->ogl.tex_null);
    if (this->atlas.default_font)
        nk_style_set_font(&this->ctx, &this->atlas.default_font->handle);
}

NK_API void
nk_sdl_handle_grab(struct nk_sdl *this)
{
    ...
    if (ctx->input.mouse.grab) {
        ...
        SDL_WarpMouseInWindow(this->win, (int)ctx->input.mouse.prev.x, (int)ctx->input.mouse.prev.y);
...
NK_API int
nk_sdl_handle_event(struct nk_sdl *this, SDL_Event *evt)
{
    struct nk_context *ctx = &this->ctx;
...
NK_API
void nk_sdl_shutdown(struct nk_sdl *this)
{
    struct nk_sdl_device *dev = &this->ogl;
